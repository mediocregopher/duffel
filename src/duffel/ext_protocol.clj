...
(defprotocol duffel-extension
    ...
    (preprocess-dir [x dir-tree]
        "Called on a dir-tree for doing transformations before the metadata templates
        get applied. Should return the transformed dir-tree. Shouldn't have side-effects")

    (preprocess-file [x file-struct]
        "Called on a file-struct for doing transformations before the metadata templates
        get applied. Should return the transformed file-struct. Shouldn't have side-effects")

    (dir-meta-tpl [x] 
        "Returns the massage template which will be applied to the metadata for dirs")

    (file-meta-tpl [x] 
        "Returns the massage template which will be applied to the metadata for files")

    (postprocess-dir [x dir-struct]
        "Called on a dir-struct (NOT dir-tree) after the massage templates are applied, before
