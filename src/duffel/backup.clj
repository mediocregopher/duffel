...
(defn prepare-backup
    "Given an absolute directory path to a file (assumes ends in slash), a filename, 
    the path to the backup root, and the number of files allowed to be backed up, creates
    the backup directory for the given file and makes sure there's no more then backup-count-1
    versions of the file already backed up"
...
(defn backup-file
    "Given an absolute directory path to a file (assumes ends in slash), a filename, 
    the path to the backup root, and the number of files allowed to be backed up, creates
    a backup file while at the same time rotating out old files"
